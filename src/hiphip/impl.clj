...
(defmacro parameterized-namespace [types & body]
  `(do ~@(for [type types]
           `(do
              (ns ~(symbol (str 'hiphip "." type))
                ~'(:refer-clojure :exclude [amap areduce alength aget aset aclone])
                ~'(:require [hiphip.impl.core :as impl]))
              (def ~'+type+ '~type)
              (import '~(symbol (str "hiphip." type "_.Helpers")))
              ~@(for [form body]
                  (w/postwalk
                   (fn [form]
                     (cond
                      (and (symbol? form)
                           (= "hiphip.impl" (namespace form)))
                      (symbol (str "hiphip." type)
                              (name form))
                      (and (symbol? form)
                           (= "Helpers" (namespace form)))
                      (symbol (str "hiphip." type "_.Helpers")
                              (name form))
                      :else
                      form))
                   form))
              (dosync
               (alter (deref (resolve 'clojure.core/*loaded-libs*)) conj '~(symbol (str 'hiphip "." type))))))))

;; As a hack to avoid writing macro-macros, this file defines the
;; per-type macros, and is loaded in each type's namespace.

(parameterized-namespace [long int double float]

 (set! *warn-on-reflection* true)
 (set! *unchecked-math* true)
 (require '[hiphip.array :as array])
